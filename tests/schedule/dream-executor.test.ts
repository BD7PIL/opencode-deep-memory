/**
 * Tests for dream-executor.
 */

import { describe, it, expect, vi } from "vitest";
import { runDream } from "../../src/schedule/dream-executor.js";

function makeMockClient(overrides: {
  createResult?: { data?: { id?: string } };
  createError?: Error;
  promptAsyncResult?: void;
  promptAsyncError?: Error;
} = {}) {
  const session = {
    create: vi.fn(),
    promptAsync: vi.fn(),
  };

  if (overrides.createError) {
    session.create.mockRejectedValue(overrides.createError);
  } else {
    session.create.mockResolvedValue(
      overrides.createResult ?? { data: { id: "dream-1" } },
    );
  }

  if (overrides.promptAsyncError) {
    session.promptAsync.mockRejectedValue(overrides.promptAsyncError);
  } else {
    session.promptAsync.mockResolvedValue(
      overrides.promptAsyncResult ?? undefined,
    );
  }

  return { session } as any; // eslint-disable-line @typescript-eslint/no-explicit-any
}

describe("runDream", () => {
  const baseOpts = {
    parentSessionID: "parent-123",
    projectPath: "/test/project",
    directory: "/test/project",
  };

  it("happy path: create returns id, promptAsync succeeds → returns spawned", async () => {
    const client = makeMockClient();

    const result = await runDream({ ...baseOpts, client });

    expect(result).toEqual({ sessionID: "dream-1", status: "spawned" });
    expect(client.session.create).toHaveBeenCalledWith({
      body: {
        parentID: "parent-123",
        title: expect.stringContaining("Memory Dream Consolidation"),
      },
      query: { directory: "/test/project" },
    });
    expect(client.session.promptAsync).toHaveBeenCalledWith({
      path: { id: "dream-1" },
      body: expect.objectContaining({
        parts: [{ type: "text", text: expect.any(String) }],
        agent: "general",
        tools: {
          memory_search: true,
          memory_store: true,
          memory_forget: false,
          read: true,
          list: true,
        },
      }),
    });
  });

  it("create() throws → returns failed, does NOT rethrow", async () => {
    const client = makeMockClient({
      createError: new Error("connection refused"),
    });

    const result = await runDream({ ...baseOpts, client });

    expect(result).toEqual({ sessionID: "", status: "failed" });
    expect(client.session.promptAsync).not.toHaveBeenCalled();
  });

  it("promptAsync() throws → returns failed", async () => {
    const client = makeMockClient({
      promptAsyncError: new Error("timeout"),
    });

    const result = await runDream({ ...baseOpts, client });

    expect(result).toEqual({ sessionID: "dream-1", status: "failed" });
  });

  it("prompt contains project path, notes file path, sessions dir substitutions", async () => {
    const client = makeMockClient();
    await runDream({ ...baseOpts, client });

    const promptCall = client.session.promptAsync.mock.calls[0][0];
    const promptText: string = promptCall.body.parts[0].text;

    // Should NOT contain template placeholders
    expect(promptText).not.toContain("{{projectPath}}");
    expect(promptText).not.toContain("{{notesFilePath}}");
    expect(promptText).not.toContain("{{sessionsDir}}");
    expect(promptText).not.toContain("{{ISO timestamp}}");

    // Should contain the actual project path
    expect(promptText).toContain("/test/project");

    // Should reference notes.md
    expect(promptText).toContain("notes.md");
  });

  it("tools object includes memory_search, memory_store, read, list", async () => {
    const client = makeMockClient();
    await runDream({ ...baseOpts, client });

    const promptCall = client.session.promptAsync.mock.calls[0][0];
    const tools = promptCall.body.tools;

    expect(tools).toEqual({
      memory_search: true,
      memory_store: true,
      memory_forget: false,
      read: true,
      list: true,
    });
  });

  it("DREAM_PROMPT_TEMPLATE contains VERIFICATION STEP", async () => {
    const { DREAM_PROMPT_TEMPLATE } = await import(
      "../../src/schedule/dream-executor.js"
    );
    expect(DREAM_PROMPT_TEMPLATE).toContain("VERIFICATION STEP");
    expect(DREAM_PROMPT_TEMPLATE).toContain(
      "read tool to check the file still exists",
    );
  });

  it("DREAM_PROMPT_TEMPLATE limits verification to 5 files", async () => {
    const { DREAM_PROMPT_TEMPLATE } = await import(
      "../../src/schedule/dream-executor.js"
    );
    expect(DREAM_PROMPT_TEMPLATE).toContain(
      "Limit verification to 5 files maximum",
    );
  });
});
